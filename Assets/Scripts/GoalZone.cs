...
public class GoalZone : MonoBehaviour
{
    ...
    {
        //Setup phase
        TagManager hitTags = null; //Create temporary variable to hold reference to collision object's tags.
    ...
    private void OnCollisionExit(Collision collision)
    {
        ...
        if ((int)hitTags.objectTag == (int)tagManager.locationTag) //Use explicit int casts to compare tag values.
        {
            ...
            if (matchingCollisionNumber == minNumMatchingObjects - 1)
            {
                //Call function to lose happiness
                loseHappiness(matchingObjectHappiness);
            }
        ...
        else if (hitTags.objectTag != TagManager.ObjectTag.None)
        {
            ...
            //Check if the minimum number of general objects has been reached. If so, add happiness.
            if (generalCollisionNumber == minNumGeneralObjects - 1)
            {
                //Call function to gain happiness
                loseHappiness(generalObjectHappiness);
            }
        }
    }
