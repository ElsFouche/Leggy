...
public class PlayerControlScript : MonoBehaviour
{
    ...
    public float minY = 0f, maxY = 5f;   // Min/Max for Y movement (if applicable)
    public float minZ = -5f, maxZ = 5f;   // Min/Max for Z movement
    ...
    void HandleMovement()
    {
        ...
        {
            Vector3 newPosition = BaseJoint.transform.position + new Vector3(-speed * Time.deltaTime, 0f, 0f);
            newPosition.x = Mathf.Clamp(newPosition.x, minX, maxX); 
            BaseJoint.transform.position = newPosition;
